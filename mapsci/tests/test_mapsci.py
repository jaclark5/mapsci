...
temperature = 273 # [K]
bead_library = {
    "CO2": {'epsilon': 353.55, 'sigma': 3.741, 'l_r': 23.0, 'l_a': 6.66, 'Sk': 1.0, 'charge': 0.0, 'dipole': 0.0, 'quadrupole': 4.62033, 'ionization_energy': 316.3969563680995, 'mass': 0.04401},
    "CH3": {'epsilon': 256.77, 'sigma': 4.0773, 'l_r': 15.05, 'l_a': 6.0, 'Sk': 0.57255, 'charge': -0.03278, 'dipole': 0.068168573, 'quadrupole': 0.060537996, 'ionization_energy': 254.80129735161324,'mass': 0.01503}
}
...
def test_curve_fit_cross_interactions(bead_library=bead_library, temperature=temperature):
    ...
    l_a = dict_cross["CO2"]["CH3"]["l_a"]
    l_r = dict_cross["CO2"]["CH3"]["l_r"]
    assert eps==pytest.approx(273.295,abs=1e-2) and l_a==pytest.approx(6.014,abs=1e-2) and l_r==pytest.approx(18.599,abs=1e-2)
